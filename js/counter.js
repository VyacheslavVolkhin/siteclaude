...
document.addEventListener('DOMContentLoaded', () => {
	...
	counters.forEach(counter => {
		...
		// Добавляем обработчик для disabled кнопки плюс
		btnPlus.addEventListener('click', function(e) {
			// Проверяем, есть ли у кнопки класс button-disabled
			if (btnPlus.classList.contains('button-disabled')) {
				// Находим родительский js-counter
				const counterContainer = this.closest('.js-counter');
				
				// Добавляем класс counter-max
				counterContainer.classList.add('counter-max');
				
				// Удаляем класс через 3 секунды
				setTimeout(() => {
					counterContainer.classList.remove('counter-max');
				}, 1000);
				
				// Предотвращаем выполнение plusAction, так как кнопка disabled
				e.stopPropagation();
			}
		});
